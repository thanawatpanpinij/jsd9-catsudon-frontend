...
function userGoal(weight, goalWeight) {
  return !goalWeight ? "Maintain" : goalWeight < weight ? "Lose" : "Gain";
}

function calculateDailyCal(tdee, goal, intensity) {
  const step = intensity === "Mild" ? 300 : intensity === "Moderate" ? 500 : 700;
  let dailyCal = tdee;
  if (goal === "Lose") {
    dailyCal = tdee - step;
  }
...
export { calculateBMR, calculateTDEE, userGoal, calculateDailyCal, calculateTotalCalories, estimateDays, calculateNutrition };

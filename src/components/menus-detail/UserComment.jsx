...
export default function UserComment({ name, imageUrl, comment, date }) {
    ...
    // function handleShowOption(e) {
    //     console.log(e.target);
    //     setShowOption(!showOption);

    //     if (showOption && e.target === document.body && e.target !== ) {
    //         setShowOption(false);
    //     }
    // }

    // useEffect(() => {
    //     document.body.addEventListener("click", handleShowOption);
    // }, [showOption]);
    return (
            ...
            <section className="flex justify-between items-center mb-4">
                <section className="flex gap-2">
                    <div className="overflow-hidden w-[50px] rounded-full">
                        <img className="w-full aspect-square object-cover" src={imageUrl} alt={`${name} picture`} />
                    </div>
                    <div>
                ...
                <div className="relative">
                    <button className="cursor-pointer" aria-label="show dropdown option">

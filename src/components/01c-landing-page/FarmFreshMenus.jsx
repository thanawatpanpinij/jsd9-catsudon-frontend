...
const FarmFreshMenus = () => {
  ...
  return (
    <section className="flex flex-col max-w-[1440px] mx-auto px-[32px] mb-[100px]">
      <div className="mb-[32px]">
        <div className="flex flex-col leading-tight">
          ...
          <div className="flex items-center justify-between max-[482px]:flex-col  max-[482px]:items-start">
            ...
            <button className="bg-primary rounded-full px-6 py-2 text-white max-[482px]:mt-4 hover:bg-third transition-all duration-200 ease cursor-pointer">
              All Menus
            </button>
          </div>
        </div>
      ...
      >
        {firstItemsByCategory.map((menu, index) => (
          <SwiperSlide key={menu.id}>
            <div className="w-[290px] border-[1.5px] border-gray-300 p-5 rounded-[32px] flex flex-col justify-center gap-3">
              <div className="flex items-center justify-between">
                ...
                <RiHeartFill className={`text-gray-300 text-normal-size cursor-pointer 
                  ${likedItems[menu.id] ? "text-secondary" : "text-gray-300"}`} 
                  onClick={() => handleLike(menu.id)}/>
              </div>

              <img
                className="w-full h-[160px] object-cover rounded-[24px]"
                src={menu.imageUrl}
                alt={menu.name}
                 loading="lazy"
              />

          ...
          {blogs.slice(1, 4).map((blog) => (
            <div key={blog.id} className="flex items-start gap-4">
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 mt-1 mb-1">{blog.contentPreview}</p>
                <p className="text-sm font-medium text-black">{blog.category}</p>
              </div>

              <img src={blog.imageUrl} alt={blog.title} className="w-24 h-24 object-cover rounded-lg flex-shrink-0" />
            </div>
          ))}
        </div>
      </div>
        ...
        {blogs.slice(4, 7).map((blog) => (
          <div key={blog.id} className="relative rounded-2xl overflow-hidden">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
              ...
              <p className="text-white text-sm font-medium">{blog.title.length > 60 ? blog.title.slice(0, 57) + "..." : blog.title}</p>
            </div>
          </div>
        ))}
      </div>
